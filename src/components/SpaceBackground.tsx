...
const SpaceBackground = ({ children }: SpaceBackgroundProps) => {
  ...
  useEffect(() => {
    ...
    const starCount = Math.min(Math.floor(containerWidth * containerHeight / 3000), 200);
    ...
    for (let i = 0; i < starCount; i++) {
      ...
      star.className = `star twinkle ${size > 0.8 ? 'star-large' : size > 0.5 ? 'star-medium' : ''}`;
      star.style.left = `${x}%`;
      star.style.top = `${y}%`;
    ...
    const addPlanetDecoration = (className: string, size: string, left: string, top: string, animDelay: string = '0s') => {
